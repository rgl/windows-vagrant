...
source "qemu" "windows-11-iot-24h2-uefi-amd64" {
  cpus   = 2
  memory = 4096
  qemuargs = [
...
source "proxmox-iso" "windows-11-iot-24h2-uefi-amd64" {
  template_name            = "template-windows-11-iot-24h2-uefi"
  template_description     = <<-EOS
                              ...
                              ```
                              EOS
  tags                     = "windows-11-iot-24h2-uefi;template"
...
source "hyperv-iso" "windows-11-iot-24h2-uefi-amd64" {
  cpus         = 2
  memory       = 4096
  generation   = 2
  boot_wait    = "1s"
...
build {
  ...
  post-processor "vagrant" {
    except               = ["proxmox-iso.windows-11-iot-24h2-uefi-amd64"]
