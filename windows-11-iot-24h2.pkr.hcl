...
variable "iso_url" {
  type    = string
  default = env("WINDOWS_11_IOT_ISO_URL")
}

variable "iso_checksum" {
  type    = string
  default = env("WINDOWS_11_IOT_ISO_CHECKSUM")
}
...
source "qemu" "windows-11-iot-24h2-amd64" {
  accelerator  = "kvm"
  machine_type = "q35"
  cpus         = 2
  memory       = 4096
  ...
  cd_files = [
    ...
    "provision-pwsh.ps1",
    "provision-winrm.ps1",
    "tmp/windows-11-iot-24h2/autounattend.xml",
  ]
  format                   = "qcow2"
  headless                 = true
...
source "proxmox-iso" "windows-11-iot-24h2-amd64" {
  template_name            = "template-windows-11-iot-24h2"
  template_description     = <<-EOS
                              ...
                              ```
                              EOS
  tags                     = "windows-11-iot-24h2;template"
...
source "hyperv-iso" "windows-11-iot-24h2-amd64" {
  cpus         = 2
  memory       = 4096
  generation   = 2
  boot_wait    = "1s"
  ...
  cd_files = [
    ...
    "provision-pwsh.ps1",
    "provision-winrm.ps1",
    "tmp/windows-11-iot-24h2-uefi/autounattend.xml",
  ]
  disk_size                = var.disk_size
...
build {
  ...
  post-processor "vagrant" {
    except               = ["proxmox-iso.windows-11-iot-24h2-amd64"]
