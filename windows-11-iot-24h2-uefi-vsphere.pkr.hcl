...
source "vsphere-iso" "windows-11-iot-24h2-uefi-amd64" {
  firmware      = "efi"
  CPUs          = 4
  RAM           = 4096
  ...
  cd_files = [
    ...
    "provision-winrm.ps1",
    "tmp/windows-11-iot-24h2-uefi-vsphere/autounattend.xml",
  ]
  iso_paths = [
    "[${var.vsphere_datastore}] iso/windows-11-iot-24h2-${basename(var.iso_url)}",
  ...
  vm_name                  = "windows-11-iot-24h2-uefi-amd64-vsphere"
  boot_wait                = "1s"
...
build {
  sources = ["source.vsphere-iso.windows-11-iot-24h2-uefi-amd64"]
